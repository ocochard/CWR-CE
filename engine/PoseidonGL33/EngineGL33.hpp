...
enum VertexShaderID
{
    ...
    VSTerrain = 3,
    VSWaterInst = 4,
    VSSkinned = 5, // GPU-skinned variant of VSTransform (BonePalette UBO)
    NVertexShaders, // 6
    VSNone = NVertexShaders
};
static_assert(NVertexShaders == 6, "vertex shader set changed");
...
// The leading pos/norm/t0/landClip fields sit at the SVertex offsets so the
// shared fill path can write both strides through an SVertex view.
struct SSkinnedVertex
{
    Vector3P pos;
    Vector3P norm;
    Poseidon::UVPair t0;
    uint8_t landClip;            // mirrors SVertex.landClip so the shared offsets hold
    uint8_t _pad[3];
...
static_assert(offsetof(SSkinnedVertex, pos) == offsetof(SVertex, pos), "skinned layout must mirror SVertex");
static_assert(offsetof(SSkinnedVertex, norm) == offsetof(SVertex, norm), "skinned layout must mirror SVertex");
static_assert(offsetof(SSkinnedVertex, t0) == offsetof(SVertex, t0), "skinned layout must mirror SVertex");
static_assert(offsetof(SSkinnedVertex, landClip) == offsetof(SVertex, landClip),
              "skinned layout must mirror SVertex");
...
class EngineGL33 : public Engine
{
  ...
  protected:
    ...
    void MarkGpuStage(const char* label) override;
