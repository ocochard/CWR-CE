...
// vsSkinned — GPU-skinned variant of vsTransform (off by default, --gpu-skinning).
// Identical UBOs / varyings / lighting body; the only difference is that
// `pos`/`normal` are first transformed by the per-vertex bone-matrix blend
// (BonePalette UBO, binding 5) before the world transform.  Kept as a full
// standalone module so vsTransform (the hot default path) is never touched.
// Bone attributes are read as integers (locations 4/5, after landClip at 3);
// the weight byte is rescaled by 1/128 (AnimationRTPair::WeightScale), matching
// the CPU quantization.  Requires the VBO to hold the bind pose (OrigPos).
...
layout(location = 2) in vec2 uv;
layout(location = 3) in uint landClip;
layout(location = 4) in uvec4 boneIdx;
layout(location = 5) in uvec4 boneWeight;

// Model-space bone matrices of the object being drawn (binding 5); re-uploaded
// per skinned draw by UploadBonePalette.
...
void main() {
    ...
    // the CPU AnimationRTPair::_weight (weight * WeightScale=128).  Unweighted
    // vertices (all weights 0) fall back to the bind pose.  Because the weights
    // sum to 1, blending then transforming reproduces the CPU
    // ApplyMatricesComplex transform-then-sum exactly.
...
const std::vector<ShaderModule>& AllShaders()
{
    static const std::vector<ShaderModule> mods = {
        ...
        {"vsSkinned", ShaderStage::Vertex, s_vsSkinnedGLSL},
